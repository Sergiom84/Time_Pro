...
2. Crea/agrega centros por cliente según las especificaciones
    ...
    with app.app_context():
        ...
        try:
            ...
            if 'center' not in inspector.get_table_names():
                print("\n❌ La tabla 'center' NO existe en Supabase")
...
def check_center_data():
    ...
    with app.app_context():
        ...
        try:
            ...
            if not rows:
                print("\n⚠️  No hay datos en la tabla center")
                return []
            ...
            print(f"\n❌ Error consultando datos: {e}")
            return []


def add_missing_centers():
    """Agrega los centros faltantes según las especificaciones"""
    with app.app_context():
        print("\n" + "=" * 60)
            ...
            for center_name in centers:
                if center_name not in existing[client_id]:
                    try:
                        ...
                        print(f"  ✅ Agregado: {center_name}")
                        added_count += 1
                    except Exception as e:
                        print(f"  ❌ Error agregando {center_name}: {e}")
                        db.session.rollback()
                else:
                    print(f"  ✓ Existente: {center_name}")

        if added_count > 0:
            db.session.commit()
            print(f"\n✅ {added_count} centros agregados exitosamente")
        else:
            print("\n✓ Todos los centros ya existen")
...
def show_centers_by_client():
    ...
    with app.app_context():
        ...
        for client in clients:
            ...
            print(f"\n📊 {client.name} (client_id={client.id})")
            if centers:
                for center_id, center_name in centers:
                    print(f"   • ID: {center_id} | {center_name}")
            else:
                print("   ⚠️  Sin centros configurados")


if __name__ == '__main__':
    print("\n🚀 Starting Center Verification and Setup Script...\n")

    # Step 1: Check table structure
    if not check_center_table_structure():
        print("\n❌ Por favor, crea la tabla 'center' en Supabase primero")
        sys.exit(1)
    ...
    print("\n" + "=" * 60)
    print("✅ VERIFICACIÓN Y SETUP COMPLETADO")
    print("=" * 60)
    print("\nPróximos pasos:")
    print("1. Actualizar modelo User para agregar center_id FK")
    print("2. Crear migraciones Alembic")
    print("3. Migrar datos de User.centro → User.center_id")
    print("4. Actualizar templates para selectores dinámicos")
    print()
